...
function pickCharacter() {
  store.pickCharacter(characterInput.value)
  store.updateUIState('characterChosen')
}

function pickQuestion(character) {
  store.pickQuestion(character)
}
...
<template>
  <div id="app" class="contain">
    <GameStart v-if="store.uiState === 'start'">
      <h2>Which hooman do you want to be?</h2>
      <p v-for="option in store.characterChoices" :key="option" class="character-choices">
        <label
          ><input type="radio" v-model="characterInput" :value="option" :id="option" />
          {{ option }}</label
        >
        <br />
      </p>
    ...
    <section v-else-if="store.uiState === 'characterChosen'">
      <svg viewBox="0 -180 1628 1180" class="main">
        ...
        <component :is="store.character" class="character-clip"></component>
      ...
      <div class="friendtalk">{{ store.questions[store.questionIndex].question }}</div>
      <div class="zombietalk">
        <p v-for="character in shuffle(store.characterChoices)" :key="character">
          <button @click="pickQuestion(character)">
            {{ store.questions[store.questionIndex][character] }}
          </button>
        </p>
      </div>
    </section>
